...
let e_newPassword = document.querySelector('#new-password');
...
e_submitBtn.addEventListener('click',function(){
    ...
    console.log(check)
      ...
      let newPassword = e_newPassword.value
    let result = check.checkReset(username,password,newPassword)
    console.log(result)
    // 如果账号密码正常，则发送请求

    if(result.bool) {
         let baseUrl = config.baseUrl
        axios.post(baseUrl + '/api/user/resetpassword', {
            username,
            password,
            newPassword
        },{
